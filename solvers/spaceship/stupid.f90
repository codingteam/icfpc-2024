...
contains
  integer function walk(ship, stars, steps) result(nsteps)
    ...
    do istar = 1, size(stars)
      ...
      if (nsteps > 2500000) error stop "too loong"
    end do
  contains
    subroutine walk_x()
      logical :: do_warp
  ...
  subroutine init_space_m()
    integer, parameter :: tr_max = 10000
    integer :: i
...
contains
  function load_stars() result(stars)
    type(star_t), allocatable :: stars(:)
    character(len=80) :: taskfile
    integer :: fd, i, N_size
    read (*,"(A80)") taskfile(1:80)
    read (*,*) N_size
    N_size = N_size + 1
    open(newunit=fd, file = trim(taskfile), form="formatted", status="old")
    allocate(stars(N_size))
    do i = 1, N_size
